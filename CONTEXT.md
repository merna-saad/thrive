...
1,449 lines, nine sections, written by reading the prototype at `4e0a65b`.
...
source wins, and it gets reported.**
...
| **5** | **Data providers, against Django** | **next** |
...
**277 tests, 11 spec files, all passing.** `svelte-check` clean over 302 files.
Contrast gate 43/43.

Roughly 8,575 lines under `frontend/src` — 5,357 source, 3,218 test.
...
it has held through four phases.
...
live counts — and both need providers.
...
## 12. The fixture student

`frontend/src/lib/data/stubProviders.ts` — **temporary, deleted in Phase 5.**
`getStudent()` keeps its `(): Promise<Student>` signature so the root `load`
does not change when the real providers land. That is the whole point of the
provider boundary: the seam is the signature, not the data.

Merna · MSBA · **17 month** track · goal "Data Scientist" · Fall 2026 ·
`programStart: 2026-08-03` · standing `onTrack`.

`programStart` is a **start** date. The finish term and the progress percentage
are both *derived* from it plus `track`; neither is stored anywhere, and
switching track moves both with no other edit.

People in the prototype's fixtures, for when providers land: **Amber Hanna**
(Graduate Student Advisor, Rady 2S111) and **Nelitza Morales** (Career Coach,
CMC / Zoom).
...
- **`AppShell` stays its own component** rather than inlined into
  ...
  about structure.
- **Probe before asserting.** Every test suite this session was written against
  observed output from a throwaway probe, not assumed behaviour. It caught real
  things twice — V8 rejecting a bad month but *rolling* a bad day, and the
  ignore store's key-space split.
- **Document defects as tests rather than fixing them out of scope.** Three
  now exist, each named as a defect record with the reason it was not fixed.
- **No Claude/Anthropic attribution anywhere** — commits, PRs, file headers.
  Verified clean across all 13 commits.
...
- Say plainly when something is a prototype or is not wired up. A placeholder
  ...
  there — which is why `AssistantConversation` has no brain and says so.
...
1. **The ignore store key-space defect** — Home and the calendar key it
...
2. **Where an `urgency: "unknown"` row goes** in a grouped list.
3. **Two product decisions parked pending real screens:** the missing year in
   `formatShortDate` / `fullLabel`, and `countdownPhrase` counting to "13
   months" with no year branch.
4. **`taskNotes` on `createOverrideStore`?** It duplicates the persistence
   logic, and the hardening it needed is exactly the drift that argues for
   collapsing it.
5. **Home's placeholder copy** — deliberately not `PagePlaceholder`.
6. **Mount `Toast`?** Store is ported and tested; one import.
7. **`useIgnoreUndo.ts`** not ported.
8. **`format.ts` still emits `"Invalid Date"` from `formatShortDate`** — the
   last unguarded function in the module.
9. **A parseable-but-wrong date still gets through** `describeDue`: V8 rolls
